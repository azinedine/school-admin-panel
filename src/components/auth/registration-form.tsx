...
import { wilayas, municipalities, institutions } from '@/data/mock-locations' // Keep locations mock for now if not ready to migrate, but subjects/levels must go
...
export function RegistrationForm({ onSuccess }: RegistrationFormProps) {
  ...
  const { data: classesList = [] } = useLevels()
  ...
  const onSubmit = (data: FormValues) => {
    ...
    const subjectNames = data.subjects?.map(id => {
      const s = subjectsList.find(sub => sub.id.toString() === id)
      return isRTL ? s?.name_ar || id : s?.name || id
    })

    const levelNames = data.levels?.map(id => {
      const l = classesList.find(c => c.id.toString() === id)
      return l?.name || id
    })
                                ...
                                render={({ field }) => {
                                  return (
                                    ...
                                    >
                                      <FormControl>
                                        <Checkbox
                                          checked={field.value?.includes(level.id.toString())}
                                          onCheckedChange={(checked) => {
                                            return checked
                                              ? field.onChange([...(field.value || []), level.id.toString()])
                                              : field.onChange(
                                                field.value?.filter(
