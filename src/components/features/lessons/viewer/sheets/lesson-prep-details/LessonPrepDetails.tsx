...
import { PrepDetailsHeader } from './PrepDetailsHeader.tsx'
import { PedagogicalGrid } from './PedagogicalGrid.tsx'
import { formatPrepDate } from './prep-details-config.ts'

export interface LessonPrepDetailsProps {
    data: LessonPreparation
}

/**
 * Lesson Prep Details Component - SOLID Architecture
 * Uses extracted header and grid components, config for status/formatting
 */
export function LessonPrepDetails({ data }: LessonPrepDetailsProps) {
    ...
    return (
        <div className="h-full flex flex-col">
            ...
            <PrepDetailsHeader data={data} />

            {/* Pedagogical Grid - Extracted Component */}
            <div className="mb-6">
                <PedagogicalGrid data={data} />
            </div>
                        ...
                        {data.learning_objectives?.length > 0 ? (
                            <ul className="space-y-2">
                                {data.learning_objectives.map((obj, i) => (
                                    <li
                                        key={i}
                                        className="flex gap-3 bg-muted/30 p-3 rounded-lg border text-sm"
                                    >
                                        <span className="font-bold text-blue-500/80 font-mono">
                                            {i + 1}
                                        </span>
                            ...
                            <div className="space-y-3">
                                ...
                                <ul className="space-y-2">
                                    {data.targeted_knowledge.map((item, i) => (
                                        <li
                                            key={i}
                                            className="flex gap-3 bg-yellow-50/50 dark:bg-yellow-900/10 p-3 rounded-lg border border-yellow-100 dark:border-yellow-900/20 text-sm"
                                        >
                                            <span className="font-bold text-yellow-600/80 font-mono">
                                                •
                                            </span>
                            ...
                            {data.teaching_methods?.length > 0 ? (
                                <div className="flex flex-wrap gap-2">
                                    {data.teaching_methods.map((method, i) => (
                                        <div
                                            key={i}
                                            className="flex items-center gap-2 px-3 py-1.5 rounded-full border bg-emerald-50/50 text-emerald-800 dark:bg-emerald-900/20 dark:text-emerald-300"
                                        >
                            ...
                            {data.resources_needed?.length > 0 ? (
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                                    {data.resources_needed.map((res, i) => (
                                        <div
                                            key={i}
                                            className="flex items-center gap-2 text-sm p-2 rounded bg-purple-50/50 dark:bg-purple-900/10"
                                        >
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="flex flex-wrap gap-2">
                                                {data.used_materials.map((mat, i) => (
                                                    <div
                                                        key={i}
                                                        className="flex items-center gap-2 px-3 py-1.5 rounded-full border bg-orange-50/50 text-orange-800 dark:bg-orange-900/20 dark:text-orange-300"
                                                    >
                        ...
                        {data.phases && data.phases.length > 0 ? (
                            <>
                                <div className="flex items-center justify-between mb-6">
                                    ...
                                    <Badge variant="outline">
                                        {data.phases.length} {t('pages.prep.phases.title', 'Phases')}
                                    </Badge>
                                </div>
                            ...
                            <>
                                <div className="flex items-center justify-between mb-6">
                                    ...
                                    <Badge variant="outline">
                                        {data.lesson_elements?.length ?? 0} Steps
                                    </Badge>
                                </div>
                                ...
                                <div className="space-y-8 relative pl-6 before:absolute before:left-[11px] before:top-3 before:bottom-3 before:w-[2px] before:bg-border">
                                    ...
                                    {(!data.lesson_elements ||
                                        data.lesson_elements.length === 0) && (
